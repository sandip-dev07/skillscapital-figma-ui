  ...
  return (
    <>
        ...
        <div className="flex-1 w-full h-full">
          <div className="grid grid-cols-2 gap-4">
            {data.features.map((item) => (
              <div
                key={item.name}
                className="aspect-square bg-white rounded-xl overflow-hidden p-4 sm:p-7 space-y-2 sm:space-y-4"
              >
                <Image
                  src={item.icon}
                  alt=""
      ...
      <div className="w-full flex items-center justify-center mt-4">
        <Button
          variant={"outline"}
          className="rounded-full text-sm bg-[#FCD980] hover:bg-[#FCD980] text-[#1A1F36] h-14 px-10 font-semibold w-fit mx-auto mt-6"
        >
